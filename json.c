...
struct JSON_Value* JSON_Value_Clone(struct JSON_Value* value)
{
  ...
  switch(value->type)
  {
    ...
    {
      clone = (struct JSON_Value*)malloc(sizeof(struct JSON_Value));
      memcpy(clone, value, sizeof(struct JSON_Value));
      break;
    }
      /* Dynamically allocated types */
    case JSON_VALUE_TYPE_STRING:
    {
      clone = JSON_Value_New_String(value->string_value);
      break;
    }
      /* Compound types */
    ...
    {
      clone = JSON_value_new(value->type);
      struct JSON_Value* child;
      struct JSON_Value** p = &clone->compound_value;
      for(child = value->compound_value; child; child = child->next_value)
      {
        struct JSON_Value* childClone = JSON_Value_Clone(child);
        *p = childClone;
        p = &(*p)->next_value;
      }
      break;
...
void JSON_Object_Set_Property(struct JSON_Value* object, const char* property, struct JSON_Value* value)
{
  ...
  if (*p == NULL)
  {
    *p = JSON_Value_New_String(property);
  }

  if ((*p)->next_value != NULL)
  {
    JSON_Value_Free((*p)->next_value);
  }
...
void JSON_Array_Append(struct JSON_Value* array, struct JSON_Value* value)
{
  ...
  for (p = &array->compound_value; *p; p = &(*p)->next_value);
...
unsigned int JSON_Array_Size(struct JSON_Value* array)
{
  ...
  unsigned int size;
  for (size = 0, p = &array->compound_value; *p; ++size, p = &(*p)->next_value);
...
struct JSON_Value* JSON_Array_Get_Value(struct JSON_Value* array, unsigned int index)
{
  ...
  unsigned int i;
  for (i = 0, p = &array->compound_value; *p && i != index; ++i, p = &(*p)->next_value);
